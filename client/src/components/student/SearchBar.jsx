...
const SearchBar = ({data}) => {
  ...
  return (
    ...
    <div className=''>
      ...
      <form onSubmit={onSearchHandler} className='max-xl w-full md:h-14 h-12 flex  items-center bg-white
      border border-gray-500/20 rounded'>

        <img src={assets.search_icon} alt="search_icon" className='md:w-auto w-10 
        px-3' />

        <input type="text" onChange={e =>SetInput(e.target.value)} value={input} placeholder='Search for courses' className='w-full h-full
